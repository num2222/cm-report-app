...
def reset_db():
    """ลบ table 'cases' เก่าทิ้งแล้วสร้างใหม่ตาม schema ปัจจุบัน
    ใช้ครั้งเดียวตอน schema เปลี่ยน (เพิ่ม column ใหม่) และข้อมูลเก่าเป็นแค่ข้อมูลทดสอบ
    เรียกผ่าน URL พร้อม query string ?key=... เพื่อป้องกันคนอื่นเรียกโดยไม่ตั้งใจ
    *** ข้อมูลทั้งหมดใน table cases จะหายถาวร กู้คืนไม่ได้ ***
    *** ลบ endpoint นี้ออกหลังใช้งานเสร็จ ***"""
    key = request.args.get('key', '')
    expected = os.environ.get('SECRET_KEY', 'cm-dev-key-change-in-prod')
    if key != expected:
        return jsonify(error='key ไม่ถูกต้อง — ดู SECRET_KEY ใน Render Environment Variables'), 403
    try:
        Case.__table__.drop(db.engine, checkfirst=True)
        db.create_all()
        return jsonify(ok=True, message='ลบและสร้าง table cases ใหม่สำเร็จ — schema ปัจจุบันมีครบทุก column แล้ว')
    except Exception as e:
