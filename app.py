...
def _write_rows(ws, cases, start_row=8):
    ...
    kpi_font      = Font(name='TH Sarabun New', size=14, bold=True, color='1E7E4A')
    kpi_font_fail = Font(name='TH Sarabun New', size=14, bold=True, color='C0392B')
    kpi_font_grey = Font(name='TH Sarabun New', size=12, color='888888')
    ...
    for i, c in enumerate(cases):
        ...
        if is_cancelled:
            # ยกเลิกใบงาน: ล้าง KPI ทั้งหมด, ใส่หมายเหตุใน col Q
            cell_k = ws.cell(row=r, column=11); cell_k.value = ''; cell_k.alignment = center
        else:
        ...
        if is_cancelled:
            cell_p = ws.cell(row=r, column=16); cell_p.value = ''; cell_p.alignment = center
            ws.cell(row=r, column=17).value = 'ยกเลิกใบงาน'  # col Q = หมายเหตุ
        else:
            kpi2_val = _kpi2_export(c)
            cell_p = ws.cell(row=r, column=16)
            cell_p.value = kpi_sym(kpi2_val)
            cell_p.font = kpi_font if kpi2_val == 'pass' else kpi_font_fail
    ...
    def write_month_rows(ws, cases):
        ...
        for i,c in enumerate(cases):
            ...
            if is_cancelled:
                ...
                ws.cell(row=r, column=18).value = 'ยกเลิกใบงาน'  # col R = หมายเหตุ
            else:
