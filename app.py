...
def _seq_num(c):
    ...
    ถ้า seq ว่างหรือไม่ใช่ตัวเลข ให้ใส่ท้ายตาราง (999999) แทนที่จะ error"""
    try:
...
def _kpi2_export(c):
    ...
    ถ้าไม่มีเวลาปิดงาน (closeTime) = ยังไม่เสร็จ = fail เสมอ
    ถ้ามีเวลาปิดงานแล้ว ใช้ค่า kpi2 จริงที่คำนวณไว้"""
    if not c.get('closeTime'):
        return 'fail'
    return c.get('kpi2', '')
