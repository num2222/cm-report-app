...
# เพิ่ม Route หน้าแรกเพื่อแก้ไข Error 405 และช่วยให้ Web App เช็คสถานะผ่านฉลุย
@app.route('/', methods=['GET'])
def index():
    return jsonify({
        "status": "online",
        "message": "CM Report Export Server is running successfully on Render!"
    }), 200

# จัดการ OPTIONS request สำหรับ Preflight CORS
...
def export_daily():
    ...
    for zname in ['MTB', 'Z2', 'FZ', 'SAT1']:
        if zname in wb.sheetnames:
            ws = wb[zname]
            ws.cell(row=4, column=5).value = fmt_be(date_iso)
    ...
    for zname in ['MTB', 'Z2', 'FZ', 'SAT1']:
        if zname in wb.sheetnames:
            ...
            for idx, c in enumerate(z_cases, start=1):
                ...
                ws.cell(row=row_idx, column=15).value = c.get('timeFinish', '')
                ws.cell(row=row_idx, column=16).value = kpi_sym(c.get('kpi2', ''))
                ws.cell(row=row_idx, column=17).value = kpi_sym(c.get('kpi3', ''))
